...
| - | - | - | - | - |
| CPU | ✔️ | ✔️ | ~200 | Simple but low performance GDScript implementation. Maintained for compatibility but not recommended. |
| GPU (GDShader) | 🚧 | 🚧 | ~5,000 | Implementation of O(n^2) using GDShaders. Slower than using compute shaders but supports Web browsers. |
| GPU (GLSL) | ✔️ | ✖️ | 8,000+ | Variant of the GPU method which uses `.glsl` compute shaders to attain absolutely maximum performance. Does not support web browsers due to Godot limitations. |
